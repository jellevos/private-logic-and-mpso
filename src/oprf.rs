...
impl<'a> OprfLeader<'a> {
    ...
    pub fn hash_elements(&mut self, leader_set: Set, bin_count: usize, hash_count: usize) -> Vec<Vec<usize>> {
        let n = self.leader.streams.len();

        // Initiate OPRFs with each assistant
        let client_blind_results_per_assistant: Vec<Vec<OprfClientBlindResult<CipherSuite>>> = self.leader.streams[0..n].iter().map(|mut assistant| {
            ...
            // Send message
            let mut message: Vec<u8> = bincode::serialize(&client_blind_results.iter().map(|client_blind_result| client_blind_result.message.serialize().to_vec()).collect::<Vec<_>>()).unwrap();
        ...
        let element_hashes_per_assistant: Vec<_> = self.leader.streams[0..n].iter().zip(client_blind_results_per_assistant).map(|(assistant, client_blind_results, )| {
            ...
            let element_hashes: Vec<Vec<u8>> = client_blind_results.iter().zip(leader_set.elements.iter()).zip(messages).map(|((client_blind_result, element,), message)| {
                ...
                client_blind_result.state.finalize(&bytes, &evaluation_element).expect("Unable to perform client finalization").to_vec()
            }).collect();

            element_hashes
        }).collect();
        
        leader_set.elements.iter().enumerate().map(|(index, element)| {
            let bytes = element.to_be_bytes();
...
impl<'a> OprfAssistant<'a> {
    ...
    pub fn hash_elements(&mut self) {
        ...
        let replies: Vec<Vec<u8>> = messages.into_iter().map(|message| {
            let server_evaluate_result = self.server.blind_evaluate(&BlindedElement::deserialize(&message).unwrap());
...
pub fn hash_leader_elements_assistant(
